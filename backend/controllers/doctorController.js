...
const loginDoctor = async (req, res) => {
    ...
    try {
        ...
        if (isMatch) {
            ...
            res.json({ success: true, token, userData: { name: user.name, email: user.email, image: user.image } })
        } else {
...
// API to add/remove unavailable dates for doctor panel
const updateUnavailableDates = async (req, res) => {
    try {
        const { docId, date } = req.body

        const docData = await doctorModel.findById(docId)
        let unavailableDates = docData.unavailableDates || []
        ...
        await doctorModel.findByIdAndUpdate(docId, { unavailableDates })
        res.json({ success: true, message: 'Unavailable dates updated' })
